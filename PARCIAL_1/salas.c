 ...
 *
 */
int getNombre(char* pResultado, int longitud,char* mensaje, char* mensajeError, int reintentos)
{
    ...
    while(reintentos>0)
    {
        ...
        else if (strnlen(bufferString,sizeof(bufferString)) > longitud-1)
        {
            printf("\nEl dato ingresado es muy largo, debe contener un maximo de %d caracteres\n",longitud);
            system("pause");
        }
        printf("%s",mensajeError);
    }
